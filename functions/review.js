...
const calculateNewRatings = (oldRatings, review, IsDelete, bIsPrice) => {
	// Object keys
	let average;
	let sum;
	let count;
	...
	ratings[average] = ratings[sum] / ratings[count]; // Average
	return ratings;
};
				...
				return db
					...
					.then((snapshot) => {
								...
								db.collection('services')
									...
									.then((doc) => {
										...
										db.collection('services')
											...
											.update({
												ratingCount:
													newRatings.ratingCount,
												ratingSum: newRatings.ratingSum,
												rating: newRatings.rating,
												priceCount:
													newPriceRatings.priceCount,
												priceSum:
													newPriceRatings.priceSum,
						...
						return res.status(422).send({
							error:
								'This account already has made a review to this service. Only 1 review per service is allowed.',
							type: 'warning'
						});
					})
					.catch((error) => {
